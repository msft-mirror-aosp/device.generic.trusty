 ...
 */
interface IMemoryBufferShare {
    /*
     * Normal Host accessible buffer.
     */
    const int HOST_BUFFER = 1;

    /*
     * Buffer for secure display use case.
     */
    const int SECURE_DISPLAY_FRAME_BUFFER = 2;

    /*
     * Buffer for secure video decoding use case.
     */
    const int SECURE_VIDEO_DECODER_INPUT = 3;
     ...
     *
     * @param protectionID:
     *      ID that describes which type of memory buffer is being shared. Valid values match the
     *      constant definitions at the top of this interface.
     *
     * @return:
     ...
     */
    ShareMemoryBufferResult shareMemoryBuffer(in ParcelFileDescriptor fd, int sizeBytes, int protectionID);
}
